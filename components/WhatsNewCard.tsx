...
const SESSION_KEY = 'cantina_whats_new_session_viewed_v2';
...
export const WhatsNewCard: React.FC<WhatsNewCardProps> = ({ onOpenChat }) => {
  ...
  return (
    <>
      {/* ================= BOLHA FLUTUANTE (Permanente no Canto Inferior Esquerdo) ================= */}
      {!isOpen && (
        <div className="fixed bottom-6 left-6 z-40 animate-fade-in">
          <button
            type="button"
            onClick={() => setIsOpen(true)}
            className="flex items-center gap-2 px-4 py-3 bg-white/95 backdrop-blur-md hover:bg-white text-emerald-800 text-xs font-black rounded-full border border-emerald-200/90 shadow-2xl hover:shadow-emerald-500/20 ring-4 ring-emerald-50 transition-all duration-200 active:scale-95 group"
            title="Ver novidades do sistema"
          >
            <span className="text-base group-hover:scale-125 transition-transform duration-200">✨</span>
            <span className="tracking-wide">Novidades (Chat & Salgadada)</span>
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
          </button>
        </div>
      )}

      {/* ================= MODAL BLOQUEANTE COM BACKDROP FOSCO ================= */}
      {isOpen && (
        <div 
          className="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-5 bg-gray-950/80 backdrop-blur-md animate-fade-in"
          // Bloqueio de cliques fora: não fecha ao clicar no backdrop para forçar ação no botão
          onClick={(e) => e.stopPropagation()}
        >
          <div 
            className="w-full max-w-2xl max-h-[92vh] flex flex-col bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden animate-scale-up"
            onClick={(e) => e.stopPropagation()}
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5 bg-gradient-to-b from-gray-50/50 to-white text-gray-800">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 text-xs">
                  <div className="bg-emerald-50/60 p-3 rounded-xl border border-emerald-100">
                    ...
                    <p className="text-gray-600 text-[11px] leading-relaxed">
                      Mensagens cifradas de ponta a ponta. Nem o servidor consegue ler o conteúdo.
                    </p>
                  </div>
